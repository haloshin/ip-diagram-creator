...
description: 个人 IP 图解创作器。用你的 IP 形象，生成专属内容图解。用于把用户授权提供的照片、主页截图、简介、账号资料或角色档案转成稳定的极简手绘图解角色，并继续生成长文配图、图文号知识卡、手机海报、方法拆解图、分镜故事图等内容视觉。用户想做个人 IP 形象、用自己的形象做知识图、图文号知识卡、文章配图、课程图解、内容海报、方法拆解图，或上传照片/主页截图/简介希望做内容视觉时，使用本 Skill。
license: MIT
---
...
本 Skill 不是头像生成器，也不是自由风格生图器。它的目标是把用户授权提供的个人材料转成一套可长期复用的极简手绘图解角色，再用这个角色参与内容表达。
...
2. **三张角色固定资产 prompt 或图片**：
   ...
   - **03 动作 / 表情 / 小比例场景扩展图**：复杂动作、表情和大场景小人参考。必须包含小比例场景角色，保证人物只占画面 10%-20% 时仍保留识别锚点。
...
- **知识卡模式**：完整内容容器，可承载标题、模块、步骤、风险、行动、金句、评分、截图批注、课程结构等。知识卡形态可参考样本，但不要被样本限制；根据内容结构、发布比例和阅读场景创造合适形态。
...
- 白底、极简手绘、留白、克制红橙蓝批注。
- 当前图类型和尺寸。
- 内容确认卡里的核心观点、图内文字、主画面隐喻、角色动作。
- 禁止 PPT、商业海报、幼态可爱、复杂背景、复刻旧图。
...
4. 图内文字是否准确、可读、不过量。
5. 风格是否仍是白底手绘、留白、克制。
6. 是否太 PPT、太商业、太幼态、太精致。
7. 是否复刻旧图构图。
8. 用户可见文字是否误写内部代称；如出现，改为 Agents、执行 Agent、用户、执行者、反馈来源等。
